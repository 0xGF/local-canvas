...
export const GAP_OPTIONS = SPACING.slice(0, 20).map(v => ({ value: v, label: `${v} (${parseFloat(v) * 4}px)` }));
...
export const SPACING_OPTIONS = SPACING.slice(0, 18).map(v => ({ value: v, label: `${v}` }));
...
// Prefix → CSS property for inline style preview
